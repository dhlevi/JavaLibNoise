...
public class Hillshader 
{
	...
	public static HSLColor shadePixel(double[][] noise, int width, int height, int x, int y, double zFactor, double Altitude, double Azimuth, double cellSize, int buffer)
	{
        ...
        if (xRateOfChange != 0) aspectRad = Math.atan2(yRateOfChange, -xRateOfChange);
        if (aspectRad < 0) aspectRad = (2 * Math.PI) + aspectRad;

        if (xRateOfChange == 0)
        {
            if (yRateOfChange > 0) aspectRad = Math.PI / 2.0;
            else if (yRateOfChange < 0) aspectRad = (2.0 * Math.PI) - (Math.PI / 2.0);
        }
