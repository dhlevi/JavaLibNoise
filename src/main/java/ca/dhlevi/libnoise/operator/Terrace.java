...
public class Terrace extends Operator
{
    ...
    public double getValue(double x, double y, double z, int scale)
    {
        ...
        if (i0 == i1)
            return getControlPoints().get(i1);
    ...
    {
        if (!getControlPoints().contains(input))
            getControlPoints().add(input);
    ...
    {
        if (steps < 2)
            throw new Exception("A minimum of two Control Points are required to process the Terrace operation.");
