...
public class Select extends Operator
{
	...
	public double getValue(double x, double y, double z, int scale)
    {
        ...
        if (fallOff > 0.0)
        {
            ...
            if (cv < (min - fallOff)) return this.getInputA().getValue(x, y, z, scale);
            ...
            if (cv < (max - fallOff)) return this.getInputB().getValue(x, y, z, scale);
        ...
        if (cv < min || cv > max) return this.getInputA().getValue(x, y, z, scale);
