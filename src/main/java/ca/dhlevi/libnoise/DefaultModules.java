...
public class DefaultModules
{
    public static Module getContinentNoise(int seed)
    {
        ...
        Turbulence distort = clamped.turbulence(15.25, 0.0087912087912088, 13, seed + 22).turbulence(47.25, 0.0023054755043228, 12, seed + 23).turbulence(95.25, 0.000980632507967639, 11, seed + 24);
        ...
        terrControlPoints.add(1.0);
        Terrace terr = continentDef.terrace(false, terrControlPoints); // this
                                                                       // terrain
                                                                       // is a
                                                                       // good
                                                                       // height
        ...
        Cache continentalShelf = new RidgedMultifractal(4.375, 2.2, 16, seed + 130, QualityMode.High).scaleBias(-0.125, -0.125).add(terr.clamp(0, -0.75)).cache();
    ...
    {
        return new Select(new Billow(2, 2, 0.5, 6, seed + 10, QualityMode.High).scaleBias(0.125, -0.75), new RidgedMultifractal(1, 2, 6, seed, QualityMode.High), new Perlin(0.5, 2, 0.25, 6, seed + 20, QualityMode.High), 0, 1000, 0.125).turbulence(4, 0.125, 6, seed + 30).cache();
    }
    ...
    public static Module getDetailedNoise(int seed)
    {
        ...
        Curve baseContinentDef_cu = new Perlin(1, 2.208984375, 0.5, 14, seed, QualityMode.Medium).curve(baseContinentDef_cuPts);
        Cache baseContinentDef = new Perlin(4.34375, 2.208984375, 0.5, 11, seed + 1, QualityMode.Medium).scaleBias(0.625, 0.375).min(baseContinentDef_cu).clamp(1, -1).cache();

        Turbulence continentDef_tu2 = new Turbulence(baseContinentDef, 15.25, 0.0087912087912088, 13, seed + 10).turbulence(95.25, 0.000980632507967639, 11, seed + 12).turbulence(47.25, 0.0023054755043228, 12, seed + 11);
        ...
        terrainTypeDef_tePnts.add(1.0);
        Cache terrainTypeDef = new Turbulence(continentDef, 18.125, 0.0485584218512898, 3, seed + 20).terrace(false, terrainTypeDef_tePnts).cache();

        ScaleBias mountainBaseDef_sb0 = new RidgedMultifractal(1723, 2.142578125, 4, seed + 30, QualityMode.Medium).scaleBias(0.5, 0.375);
        ScaleBias mountainBaseDef_sb1 = new RidgedMultifractal(367, 2.142578125, 1, seed + 31, QualityMode.High).scaleBias(-2, -0.5);

        Cache mountainBaseDef = new Blend(new Const(-1), mountainBaseDef_sb0, mountainBaseDef_sb1).turbulence(1337, 0.000148588410104012, 4, seed + 32).turbulence(21221, 8.32244480138485E-06, 6, seed + 33).cache();
        ...
        Cache mountainousHigh = new Max(mountainousHigh_rm0, mountainousHigh_rm1).turbulence(21511, 5.5441284907219E-06, 4, seed + 42).cache();
        ...
        Add mountainousTerrain_ad = mountainousHigh.scaleBias(0.25, 0.25).add(mountainBaseDef);
        Select mountainousTerrain_se = new Select(mountainousTerrain_sb0, mountainousTerrain_ad, mountainBaseDef, -0.5, 999.5, 0.5);

        Cache mountainousTerrain = mountainousTerrain_se.scaleBias(0.8, 0).exponent(1.375).cache();
        ...
        Cache hillyTerrain = hillyTerrain_bl.scaleBias(0.75, -0.25).exponent(1.375).turbulence(1531, 5.90981620471603E-05, 4, seed + 62).turbulence(21617, 8.50853831820232E-06, 6, seed + 63).cache();
        ...
        Cache plainsTerrain = plainsTerrain_sb0.multiply(plainsTerrain_sb1).scaleBias(2, -1).cache();

        ScaleBias badlandsSand_sb1 = new Voronoi(16183.25, 0, seed + 81, true).scaleBias(0.25, 0.25);
        Cache badlandsSand = new RidgedMultifractal(6163.5, 2, 1, seed + 80, QualityMode.High).scaleBias(1, 0).add(badlandsSand_sb1).cache();
        ...
        Cache badlandsCliffs = new Perlin(839, 2.212890625, 0.5, 6, seed + 90, QualityMode.High).curve(badlandsCliffs_cuPts).clamp(0.875, -999.125).terrace(false, badlandsCliffs_tePts).turbulence(16111, 7.06519051286218E-06, 3, seed + 91).turbulence(36107, 4.72717130796103E-06, 3, seed + 92).cache();
        ...
        ScaleBias scaledMountainousTerrain_sb99 = new Perlin(14.5, 2.142578125, 0.5, 6, seed + 110, QualityMode.High).exponent(1.25).scaleBias(0.25, 1);
        ...
        Cache continentalShelf = new RidgedMultifractal(4.375, 2.208984375, 16, seed + 130, QualityMode.High).scaleBias(-0.125, -0.125).add(continentalShelf_cl).cache();
        ...
        Cache riverPoisitions = riverPositions_cu0.min(riverPositions_cu1).turbulence(9.25, 0.0173160173160173, 6, seed + 102).cache();
